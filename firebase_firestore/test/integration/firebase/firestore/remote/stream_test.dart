...
void main() {
  ...
  Future<void> waitForWriteStreamOpen(AsyncQueue testQueue, WriteStream writeStream, _StreamStatusCallback callback) async {
  ...
  Future<WriteStream> createAndOpenWriteStream(AsyncQueue testQueue, _StreamStatusCallback callback) async {
    final Datastore datastore = Datastore(IntegrationTestUtil.testEnvDatabaseInfo(), testQueue, EmptyCredentialsProvider());
  ...
  test('testStreamRefreshesTokenUponExpiration', () async {
    ...
    final Datastore datastore = Datastore(IntegrationTestUtil.testEnvDatabaseInfo(), testQueue, mockCredentialsProvider);
    final _StreamStatusCallback callback = _StreamStatusCallback();
    final WriteStream writeStream = datastore.createWriteStream(callback);
    await waitForWriteStreamOpen(testQueue, writeStream, callback);
    ...
    expect(mockCredentialsProvider.states, orderedEquals(<String>['getToken', 'invalidateToken', 'getToken', 'getToken']));
  });
}
...
class _StreamStatusCallback implements WatchStreamCallback, WriteStreamCallback {
  ...
  factory _StreamStatusCallback() {
    ...
    return _StreamStatusCallback._(
      ...
      onWatchChange: (SnapshotVersion snapshotVersion, WatchChange watchChange) async => watchChangeCompleter.complete(),
      onClose: (GrpcError error) async => closeCompleter.complete(),
      onWriteResponse: (SnapshotVersion commitVersion, List<MutationResult> mutationResults) async => responseReceivedCompleter.complete(),
