...
/// An [WatchChangeExistenceFilterWatchChange] applies to the targets and is required to verify the
/// current client state against expected state sent from the server.
...
class WatchChangeDocumentChange extends WatchChange {
  const WatchChangeDocumentChange(
      this.updatedTargetIds, this.removedTargetIds, this.documentKey, this.newDocument)
      : super._();

  /// The target IDs for which this document should be updated/added. The new document applies to
  /// all of these targets.
  // TODO: figure out if we can actually use arrays here for efficiency
  final List<int> updatedTargetIds;

  /// The new document is removed from all of these targets. The target IDs for which this document
...
class WatchChangeWatchTargetChange extends WatchChange {
  WatchChangeWatchTargetChange(
    ...
    GrpcError cause,
        ...
        // We can get a cause that is considered ok, but everywhere we assume that any non-null
        // cause is an error.
