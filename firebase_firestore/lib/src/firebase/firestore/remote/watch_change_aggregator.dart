...
class WatchChangeAggregator {
  ...
  /// Processes and adds the [WatchChangeDocumentChange] to the current set of changes.
  ...
  /// Processes and adds the [WatchChangeWatchTargetChange] to the current set of changes.
    ...
    for (int targetId in _getTargetIds(targetChange)) {
      ...
      switch (targetChange.changeType) {
        ...
        case WatchTargetChangeType.removed:
          ...
          // changes. We need to decrement the number of pending acks needed from watch for this
          // targetId.
          targetState.recordTargetResponse();
          if (!targetState.isPending()) {
  ...
  /// Returns all [targetIds] that the watch change applies to: either the [targetIds] explicitly
  /// listed in the change or the [targetIds] of all currently active targets.
  ...
  /// Adds the provided document to the internal list of document updates and its document key to
  /// the given target's mapping.
  ...
  /// Removes the provided document from the target mapping. If the document no inter matches the
