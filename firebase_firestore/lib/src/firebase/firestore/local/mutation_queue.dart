...
abstract class MutationQueue {
  ...
  /// invariants, etc.
  ///
  /// After starting, the mutation queue must guarantee that the [highestAcknowledgedBatchId] is
  ...
  // TODO: PERF: Current consumer only needs mutated keys; if we can provide that cheaply, we should
  //  replace this.
  Future<List<MutationBatch>> getAllMutationBatches();

  /// Finds all mutation batches that could <b>possibly<b> affect the given document key. Not all
  ...
  ///
  /// Note that because of this requirement implementations are free to return mutation batches
  ...
  /// Finds all mutation batches that could <b>possibly<b> affect the given set of document keys.
  /// Not all mutations in a batch will necessarily affect each key, so when looping through the
  /// batch you'll need to check that the mutation itself matches the key.
  ///
  /// Note that because of this requirement implementations are free to return mutation batches
  ...
  /// Batches are guaranteed to be sorted by batch ID.
  ///
  /// NOTE: A [PatchMutation] does not need to include all fields in the query filter criteria in
