...
enum ConflictAlgorithm {
  /// When a constraint violation occurs, an immediate ROLLBACK occurs, thus ending the current
  /// transaction, and the command aborts with a return code of SQLITE_CONSTRAINT. If no transaction
  /// is active (other than the implied transaction that is created on every command) then this
  ...
  /// the row that contained the constraint violation continue to be inserted or updated normally.
  ...
  /// returned. If a NOT NULL constraint violation occurs, the NULL value is replaced by the default
  /// value for that column. If the column has no default value, then the ABORT algorithm is used.
  /// If a CHECK constraint violation occurs then the IGNORE algorithm is used. When this conflict
  /// resolution strategy deletes rows in order to satisfy a constraint, it does not invoke delete
  /// triggers on those rows. This behavior might change in a future release.
  replace,
}
