...
abstract class TargetCache {
  ...
  /// Returns the highest sequence number that the cache has seen. This includes
  /// targets that have been persisted in a previous run of the client.
  ...
  Future<void> forEachTarget(Consumer<TargetData> consumer);

  /// A global snapshot version representing the last consistent snapshot we
  /// received from the backend. This is monotonically increasing and any
  /// snapshots received from the backend prior to this version (e.g. for
  /// targets resumed with a 'resume_token') should be suppressed (buffered)
  /// until the backend has caught up to this snapshot version again. This
  /// prevents our cache from ever going backwards in time.
  ///
  /// This is updated whenever we get a [TargetChange] with a 'read_time' and
  /// empty 'target_ids'.
  ...
  ///
  /// The cache key is extracted from [TargetData.target].
  Future<void> addTargetData(TargetData queryData);

  /// Replaces an entry in the cache. An entry with the same key should already
  /// exist.
  ///
  /// The cache key is extracted from [TargetData.target].
  Future<void> updateTargetData(TargetData queryData);
  ...
  Future<void> removeTargetData(TargetData queryData);

  /// Looks up a [TargetData] entry in the cache.
  ///
  /// The [target] corresponding to the entry to look up. Returns the cached
  /// [TargetData] entry, or null if the cache has no entry for the query.
  Future<TargetData> getTargetData(Target target);
