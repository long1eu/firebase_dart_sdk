...
class LruGarbageCollector {
  ...
  LruGarbageCollectorScheduler newScheduler(AsyncQueue scheduler, LocalStore localStore) {
  ...
  Future<int> getNthSequenceNumber(int count) async {
    ...
    await delegate.forEachTarget((TargetData targetData) => buffer.addElement(targetData.sequenceNumber));
...
class LruGarbageCollectorScheduler implements GarbageCollectionScheduler {
  ...
  void _scheduleGC() {
    ...
    _gcTask = _scheduler.enqueueAfterDelay(
      TimerId.garbageCollection,
      delay,
      () async {
