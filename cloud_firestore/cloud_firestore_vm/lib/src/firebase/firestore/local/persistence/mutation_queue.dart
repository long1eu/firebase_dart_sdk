...
abstract class MutationQueue {
  ...
  /// Returns the largest (latest) batch id in mutation queue for the current user that is pending
  /// server response, [MutationBatch.unknown] if the queue is empty.
  Future<int> getHighestUnacknowledgedBatchId();
  ...
  ///
  /// Batches are guaranteed to be sorted by batch ID.
  Future<List<MutationBatch>> getAllMutationBatchesAffectingDocumentKeys(Iterable<DocumentKey> documentKeys);
