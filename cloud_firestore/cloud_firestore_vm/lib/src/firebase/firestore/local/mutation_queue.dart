...
abstract class MutationQueue {
  ...
  /// Returns the first unacknowledged mutation batch after the passed in [batchId] in the mutation queue or null if
  /// empty.
  ///
  /// [batchId] to search after, or [MutationBatch.unknown] for the first mutation in the queue.
  ///
  ...
  // TODO(long1eu): PERF: Current consumer only needs mutated keys; if we can provide that cheaply, we should replace
  //  this.
  Future<List<MutationBatch>> getAllMutationBatches();

  /// Finds all mutation batches that could <b>possibly<b> affect the given document key. Not all mutations in a batch
  /// will necessarily affect the document key, so when looping through the batch you'll need to check that the mutation
  /// itself matches the key.
  ///
  /// Note that because of this requirement implementations are free to return mutation batches that don't contain the
  ...
  /// itself matches the key.
  ///
  /// Note that because of this requirement implementations are free to return mutation batches that don't contain any
  /// of the document keys at all if it's convenient.
  ///
  /// Batches are guaranteed to be sorted by batch ID.
  Future<List<MutationBatch>> getAllMutationBatchesAffectingDocumentKeys(
      Iterable<DocumentKey> documentKeys);

  /// Finds all mutation batches that could affect the results for the given query. Not all mutations in a batch will
  /// necessarily affect the query, so when looping through the batch you'll need to check that the mutation itself
  /// matches the query.
  ///
  /// Note that because of this requirement implementations are free to return mutation batches that don't match the
  /// query at all if it's convenient.
  ///
  /// Batches are guaranteed to be sorted by batch ID.
  ///
  /// NOTE: A [PatchMutation] does not need to include all fields in the query filter criteria in order to be a match
  /// (but any fields it does contain do need to match).
