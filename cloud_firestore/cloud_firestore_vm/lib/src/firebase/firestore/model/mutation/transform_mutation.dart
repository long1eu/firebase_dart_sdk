...
class TransformMutation extends Mutation {
  ...
  TransformMutation(DocumentKey key, this.fieldTransforms) : super(key, Precondition(exists: true));
  ...
  MaybeDocument applyToRemoteDocument(MaybeDocument maybeDoc, MutationResult mutationResult) {
    ...
    return Document(key, mutationResult.version, newData, DocumentState.committedMutations);
  }
  ...
  MaybeDocument applyToLocalView(
    ...
    final Document doc = _requireDocument(maybeDoc);
    final List<FieldValue> transformResults = _localTransformResults(localWriteTime, baseDoc);
    final ObjectValue newData = _transformObject(doc.data, transformResults);
  ...
  Document _requireDocument(MaybeDocument maybeDoc) {
    ...
    hardAssert(doc.key == key, 'Can only transform a document with the same key');
    return doc;
  }
  ...
  List<FieldValue> _serverTransformResults(
    ...
    final List<FieldValue> transformResults = List<FieldValue>(fieldTransforms.length);
    hardAssert(
  ...
  ///
  /// Returns the transform results list.
  List<FieldValue> _localTransformResults(Timestamp localWriteTime, MaybeDocument baseDoc) {
    final List<FieldValue> transformResults = List<FieldValue>(fieldTransforms.length);
    int i = 0;
    for (FieldTransform fieldTransform in fieldTransforms) {
      ...
      transformResults[i] = transform.applyToLocalView(previousValue, localWriteTime);
      i++;
    }
    return transformResults;
  }

  ObjectValue _transformObject(ObjectValue objectValue, List<FieldValue> transformResults) {
    hardAssert(
        transformResults.length == fieldTransforms.length, 'Transform results length mismatch.');
  ...
  @override
  int get hashCode =>
      const ListEquality<FieldTransform>().hash(fieldTransforms) ^ keyAndPreconditionHashCode();
