...
///
/// The client constructs two generators, one for the query cache (via [forTargetCache]), and one
...
class TargetIdGenerator {
  ...
  TargetIdGenerator(int generatorId, int seed)
      : assert((generatorId & _reservedBits) == generatorId,
        ...
        assert((seed & _reservedBits) == generatorId, 'Cannot supply target ID from different generator ID'),
        _nextId = seed;

  /// Creates and returns the [TargetIdGenerator] for the local store.
  factory TargetIdGenerator.forTargetCache(int after) {
    return TargetIdGenerator(_queryCacheId, after)
