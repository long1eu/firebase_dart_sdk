...
import 'package:cloud_firestore_vm/src/firebase/firestore/model/field_path.dart' as core;
...
import 'package:cloud_firestore_vm/src/firebase/firestore/model/values.dart' as core;
...
/// ordering.
///
/// **Subclassing Note**: Cloud Firestore classes are not meant to be subclassed except for use in test mocks. Subclassing is
...
class Query {
  ...
  Query whereNotEqualTo(String field, Object value) {
    return _whereHelper(FieldPath.fromDotSeparatedPath(field), FilterOperator.notEqual, value);
  }
  ...
  ///
  /// Returns the created [Query].
  Query whereNotEqualToField(FieldPath fieldPath, Object value) {
    return _whereHelper(fieldPath, FilterOperator.notEqual, value);
  }
  ...
  Query whereArrayContainsAny(String field, List<Object> values) {
    return _whereHelper(FieldPath.fromDotSeparatedPath(field), FilterOperator.arrayContainsAny, values);
  }
  ...
  /// A Query can have only one [whereArrayContainsAny] filter and it cannot be
  /// combined with [whereArrayContains] or [whereIn].
  Query whereArrayContainsAnyField(FieldPath fieldPath, List<Object> values) {
    return _whereHelper(fieldPath, FilterOperator.arrayContainsAny, values);
  }
  ...
  Query whereIn(String field, List<Object> values) {
    return _whereHelper(FieldPath.fromDotSeparatedPath(field), FilterOperator.IN, values);
  }
  ...
  /// A Query can have only one [whereIn] filter, and it cannot be combined with
  /// [whereArrayContainsAny].
  Query whereInField(FieldPath fieldPath, List<Object> values) {
    return _whereHelper(fieldPath, FilterOperator.IN, values);
  }

  /// Creates and returns a new [Query] with the additional filter that documents must contain
  /// the specified field and the value does not equal any of the values from the provided list.
  ///
  /// One special case is that [whereNotIn] cannot match [null] values. To query for documents
  /// where a field exists and is [null], use [whereNotEqualTo], which can
  /// handle this special case.
  ///
  /// A Query can have only one [whereIn] filter, and it cannot be combined with
  /// [whereArrayContainsAny].
  Query whereNotIn(String field, List<Object> values) {
    return _whereHelper(FieldPath.fromDotSeparatedPath(field), FilterOperator.notIn, values);
  }
  ...
  /// documents where a field exists and is [null], use [whereNotEqualTo], which can handle this
  /// special case.
  ///
  /// A Query can have only one [whereIn] filter, and it cannot be combined with
  /// [whereArrayContainsAny].
  Query whereNotInField(FieldPath fieldPath, List<Object> values) {
    return _whereHelper(fieldPath, FilterOperator.notIn, values);
  }
  ...
  Query _whereHelper(FieldPath fieldPath, FilterOperator op, dynamic value) {
    ...
    if (internalPath.isKeyField) {
      if (op == FilterOperator.arrayContains || op == FilterOperator.arrayContainsAny) {
        throw ArgumentError('Invalid query. You can\'t perform "$op" queries on FieldPath.documentId().');
      } else if (op == FilterOperator.IN || op == FilterOperator.notIn) {
        _validateDisjunctiveFilterElements(value, op);
        fieldValue = Value(arrayValue: pb.ArrayValue(values: value.map(_parseDocumentIdValue).toList()));
      } else {
        fieldValue = _parseDocumentIdValue(value);
      }
    } else {
      if (op == FilterOperator.IN || op == FilterOperator.notIn || op == FilterOperator.arrayContainsAny) {
        _validateDisjunctiveFilterElements(value, op);
      }
      fieldValue = firestore.userDataReader.parseQueryValue(
        value,
        op == FilterOperator.IN || op == FilterOperator.notIn,
      );
    }
  ...
  Value _parseDocumentIdValue(Object value) {
    if (value is String) {
      if (value.isEmpty) {
        throw ArgumentError(
      ...
      final ResourcePath path = query.path.appendField(ResourcePath.fromString(value));
      ...
      return core.refValue(firestore.databaseId, DocumentKey.fromPath(path));
    } else if (value is DocumentReference) {
      return core.refValue(firestore.databaseId, value.key);
    } else {
      throw ArgumentError('Invalid query. When querying with FieldPath.documentId() you must provide a valid String or '
  ...
  void _validateDisjunctiveFilterElements(Object value, FilterOperator op) {
    if (value is List) {
      if (value.isEmpty) {
        ...
        throw ArgumentError('Invalid Query. "$op" filters support a maximum of 10 elements in the value array.');
      }
    } else {
      throw ArgumentError('Invalid Query. A non-empty array is required for "$op" filters.');
    }
  ...
  ///   * Only one array operator is allowed.
  ///   * Only one disjunctive operator is allowed.
  ///   * NOT_EQUAL cannot be used with another NOT_EQUAL operator.
  ///   * NOT_IN cannot be used with array, disjunctive, or NOT_EQUAL operators.
  ///
  /// Array operators: [FilterOperator.arrayContains], [FilterOperator.arrayContainsAny]
  /// Disjunctive operators: [FilterOperator.IN], [FilterOperator.arrayContainsAny], [FilterOperator.notIn]
  List<FilterOperator> _conflictingOps(FilterOperator op) {
    switch (op) {
  ...
  void _validateNewFilter(Filter filter) {
    if (filter is FieldFilter) {
      ...
      if (filter.isInequality) {
        ...
        if (existingInequality != null && existingInequality != newInequality) {
          throw ArgumentError(
            ...
            'lessThanOrEqualTo, greaterThan, or greaterThanOrEqualTo) must be on the same field. But you have filters on '
      ...
      final FilterOperator conflictingOp = query.findFilterOperator(_conflictingOps(filterOp));
      if (conflictingOp != null) {
        // We special case when it's a duplicate op to give a slightly clearer error message.
        if (conflictingOp == filterOp) {
          throw ArgumentError("Invalid Query. You cannot use more than one '$filterOp' filter.");
        } else {
          throw ArgumentError("Invalid Query. You cannot use '$filterOp' filters with '$conflictingOp' filters.");
        }
      }
    }
  }
  ...
  Query limit(int limit) {
    ...
    return Query(query.limitToFirst(limit), firestore);
  }

  /// Creates and returns a new [Query] that only returns the last matching documents up to the specified number.
  ///
  /// You must specify at least one [orderBy] clause for [limitToLast] queries, otherwise an exception will be thrown
  /// during execution.
  ///
  /// [limit] the maximum number of items to return.
  ///
  ...
  Query limitToLast(int limit) {
    ...
    return Query(query.limitToLast(limit), firestore);
  }
  ...
  Bound _boundFromDocumentSnapshot(String methodName, DocumentSnapshot snapshot, bool before) {
    ...
    final List<Value> components = <Value>[];
    ...
    for (OrderBy orderBy in query.orderByConstraints) {
      if (orderBy.field == core.FieldPath.keyPath) {
        components.add(refValue(firestore.databaseId, document.key));
      } else {
        final Value value = document.getField(orderBy.field);
  ...
  Bound _boundFromFields(String methodName, List<Object> values, bool before) {
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      if (orderBy.field == core.FieldPath.keyPath) {
        ...
        final ResourcePath path = query.path.appendField(ResourcePath.fromString(documentId));
        if (!DocumentKey.isDocumentKey(path)) {
          throw ArgumentError(
        ...
        components.add(core.refValue(firestore.databaseId, key));
      } else {
        final Value wrapped = firestore.userDataReader.parseQueryValue(rawValue);
  ...
  Future<QuerySnapshot> get([Source source = Source.defaultSource]) async {
    _validateHasExplicitOrderByForLimitToLast();
  ...
  void _validateHasExplicitOrderByForLimitToLast() {
    if (query.hasLimitToLast && query.explicitSortOrder.isEmpty) {
      throw StateError('limitToLast() queries require specifying at least one orderBy() clause');
    }
  }
