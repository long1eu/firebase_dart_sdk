...
import 'package:cloud_firestore_vm/src/firebase/firestore/remote/stream.dart';
...
abstract class AbstractStream<ReqT extends GeneratedMessage, RespT extends GeneratedMessage,
  ...
  AbstractStream(
    ...
    this.listener,
          ...
          _workerQueue,
          connectionTimerId,
          initialDelay: _backoffInitialDelayMs,
          backoffFactor: _backoffFactor,
          maxDelay: _backoffMaxDelayMs,
        );

  /// Initial backoff time in milliseconds after an error. Set to 1s according to
  /// https://cloud.google.com/apis/design/errors.
  static const Duration _backoffInitialDelayMs = Duration(seconds: 1);
  static const Duration _backoffMaxDelayMs = Duration(minutes: 1);
  ...
  Future<void> start() async {
    ...
    final CloseGuardedRunner closeGuardedRunner = CloseGuardedRunner(_workerQueue, _closeCount, () => _closeCount);
...
class CloseGuardedRunner {
  ...
  Future<void> run(Task<void> task) async {
    if (closeCount() == initialCloseCount) {
      await asyncQueue.enqueue(task);
    } else {
...
class StreamObserver<ReqT extends GeneratedMessage, RespT extends GeneratedMessage, CallbackT extends StreamCallback>
  ...
  @override
  void onOpen() {
    _dispatcher.run(() async => Log.d('AbstractStream', '($hashCode) Stream is open'));
  }
