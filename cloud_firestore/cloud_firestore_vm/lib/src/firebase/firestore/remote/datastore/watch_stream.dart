...
class WatchStream extends BaseStream<proto.ListenRequest, proto.ListenResponse> {
  factory WatchStream({
  ...
  WatchStream.test(
    ...
    AsyncQueue workerQueue,
        ...
        super(eventsController, workerQueue, TimerId.listenStreamIdle, TimerId.listenStreamConnectionBackoff);

  final FirestoreClient _client;
  final RemoteSerializer _serializer;

  @override
  Future<ResponseStream<proto.ListenResponse>> _buildCall(Stream<proto.ListenRequest> requests) async {
    return _client.listen(requests);
  }

  /// Registers interest in the results of the given query.
  ///
  /// If the query includes a [resumeToken] it will be included in the request. Results that affect the query will be
  /// streamed back as [WatchChange] messages that reference the [targetId] included in query.
  ...
  void _onData(proto.ListenResponse response) {
    ...
    final SnapshotVersion snapshotVersion = _serializer.decodeVersionFromListenResponse(response);
