...
extension MapExtension on Map<String, String> {
  List<String> get pairs {
    return keys.map((String key) => '$key=${Uri.encodeQueryComponent(this[key])}').toList()..sort();
  }

  List<String> pairsWhere(bool Function(String key) test) {
    ;
    return keys.where(test).map((String key) => '$key=${Uri.encodeQueryComponent(this[key])}').toList()..sort();
  }
}
